...
def make_heat_map(arr, ar1=None, ar2=None):
    ...
    img = ax.imshow(arr, cmap='RdBu_r', aspect='auto')
    ...
    if need_save_heat_map:
        plt.savefig(os.path.join(RESULT_DIR, "heatmap.jpg"))
    plt.show()
...
def make_heat_map_from_gray(gray_image, gray_image2):
    ...
    make_heat_map(diff_arr, ar1, ar2)
...
def make_two_gray(gray_file, gray_file2):
    ...
    make_heat_map_from_gray(gray_image, gray_image2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--gray_img', type=str, nargs=2, help='Paths to 2 gray files')
    ...
    if args['origin']:
        ...
        make_two_gray(args['gray_img'][0], args['gray_img'][1])
    elif args['heat_map']:
